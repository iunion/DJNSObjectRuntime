...
@implementation NSArray (MQ)

- (nullable NSArray *)makeTagsWithMaxCount:(NSUInteger)MaxCount maxLength:(NSUInteger)maxLength
{
    NSUInteger count = 0;
    NSUInteger length = 0;
    
    NSMutableArray *tages = [NSMutableArray arrayWithCapacity:0];
    
    for (NSUInteger index=0; index<self.count; index++)
    {
        NSString *tag = self[index];
        if ([tag isKindOfClass:[NSString class]])
        {
            length += tag.length;
            if (length <= maxLength && count < MaxCount)
            {
                [tages addObject:tag];
                count++;
            }
